...
SHORT_RSI_MIN    = 40      # RSI must be in this band for SHORT
SHORT_RSI_MAX    = 65
LONG_RSI_MIN     = 35      # RSI must be in this band for LONG
LONG_RSI_MAX     = 60
...
def get_signal(df: pd.DataFrame, exchange=None) -> dict:
    ...
    print(f"  EMA20={ema20:.2f}  EMA50={ema50:.2f}  RSI={rsi:.2f}")
    ...
    # ------------------------------------------------------------------
    sc1 = ema20 < ema50
    sc2 = h > ema20 and c < ema20
    sc3 = upper_wick > wick_min
    sc4 = SHORT_RSI_MIN <= rsi <= SHORT_RSI_MAX

    print(f"\n  SHORT conditions:")
    print(f"  {_pf(sc1)} C1 trend:      EMA20 ({ema20:.2f}) < EMA50 ({ema50:.2f})")
    print(f"  {_pf(sc2)} C2 rejection:  high ({h:.2f}) > EMA20, close ({c:.2f}) < EMA20")
    print(f"  {_pf(sc3)} C3 upper wick: {upper_wick:.2f} > {wick_min:.2f}")
    print(f"  {_pf(sc4)} C4 RSI:        {rsi:.2f} in [{SHORT_RSI_MIN}–{SHORT_RSI_MAX}]")

    # ------------------------------------------------------------------
    # Evaluate LONG conditions
    # ------------------------------------------------------------------
    lc1 = ema20 > ema50
    lc2 = lo < ema20 and c > ema20
    lc3 = lower_wick > wick_min
    lc4 = LONG_RSI_MIN <= rsi <= LONG_RSI_MAX

    print(f"\n  LONG conditions:")
    print(f"  {_pf(lc1)} C1 trend:      EMA20 ({ema20:.2f}) > EMA50 ({ema50:.2f})")
    print(f"  {_pf(lc2)} C2 rejection:  low ({lo:.2f}) < EMA20, close ({c:.2f}) > EMA20")
    print(f"  {_pf(lc3)} C3 lower wick: {lower_wick:.2f} > {wick_min:.2f}")
    print(f"  {_pf(lc4)} C4 RSI:        {rsi:.2f} in [{LONG_RSI_MIN}–{LONG_RSI_MAX}]")
    ...
    if sc1 and sc2 and sc3 and sc4:
        ...
        reason = (
            ...
            f"RSI {rsi:.2f} in [{SHORT_RSI_MIN}–{SHORT_RSI_MAX}], "
    ...
    if lc1 and lc2 and lc3 and lc4:
        ...
        reason = (
            ...
            f"RSI {rsi:.2f} in [{LONG_RSI_MIN}–{LONG_RSI_MAX}], "
    ...
    # ------------------------------------------------------------------
    # === NO_TRADE — explain every failed condition ===
    # ------------------------------------------------------------------
    short_fails = []
    ...
    if not sc4: short_fails.append(
        f"RSI {rsi:.2f} outside [{SHORT_RSI_MIN}–{SHORT_RSI_MAX}]")
    ...
    if not lc4: long_fails.append(
        f"RSI {rsi:.2f} outside [{LONG_RSI_MIN}–{LONG_RSI_MAX}]")

    parts = []
    if short_fails: parts.append("SHORT failed: " + "; ".join(short_fails))
    if long_fails:  parts.append("LONG failed: "  + "; ".join(long_fails))

    reason = " | ".join(parts) or "No conditions met."
    print(f"\n  [NO_TRADE] {reason}")
    return _no_trade(reason, funding_rate=funding_rate)
